...
Dir.chdir 'diego-release' do
  ...
  system(%(git status && ./scripts/sync-package-specs && git status && git diff && bosh --parallel 10 sync blobs && bosh create release --force --with-tarball --name diego --version #{version})) || raise('cannot create diego-release')
end
